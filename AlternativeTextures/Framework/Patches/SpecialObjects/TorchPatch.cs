        ...
        private static bool DrawPrefix(Torch __instance, Vector2[] ___ashes, SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
        {
            if (__instance.modData.ContainsKey(ModDataKeys.ALTERNATIVE_TEXTURE_NAME))
            {
                ...
                if (!__instance.bigCraftable)
                {
                    ...
                    spriteBatch.Draw(textureModel.GetTexture(textureVariation), position2, sourceRectangle, white, 0f, zero, (__instance.scale.Y > 1f) ? __instance.getScale().Y : 4f, SpriteEffects.None, (float)__instance.GetBoundingBoxAt(x, y).Bottom / 10000f);
                    ...
                    sourceRect.Width = 8;
                    sourceRect.Height = 8;
                    spriteBatch.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32 + 4, y * 64 + 16 + 4)), sourceRect, Color.White * 0.75f, 0f, new Vector2(4f, 4f), 3f, SpriteEffects.None, (float)(__instance.GetBoundingBoxAt(x, y).Bottom + 1) / 10000f);

                    for (int i = 0; i < ___ashes.Length; i++)
                    {
                        spriteBatch.Draw(Game1.objectSpriteSheet, Game1.GlobalToLocal(Game1.viewport, new Vector2((float)(x * 64 + 32) + ___ashes[i].X, (float)(y * 64 + 32) + ___ashes[i].Y)), new Rectangle(344 + i % 3, 53, 1, 1), Color.White * 0.5f * ((-100f - ___ashes[i].Y / 2f) / -100f), 0f, Vector2.Zero, 3f, SpriteEffects.None, (float)__instance.GetBoundingBoxAt(x, y).Bottom / 10000f);
                    }
                    return false;
                }
